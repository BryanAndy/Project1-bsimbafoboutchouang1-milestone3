...
from lyrics import get_lyrics_link
from artists import get_access_token, get_song_data
...
def index():
    ...
    DATA["artists"] = artists
    DATA["has_artists_saved"] = has_artists_saved
    DATA["song_name"] = song_name
    DATA["song_artist"] = song_artist
    DATA["song_image_url"] = song_image_url
    DATA["preview_url"] = preview_url
    DATA["genius_url"] = genius_url
...
@app.route("/save_artist", methods=["POST"])
def save_artist():
    ...
    db.session.commit()
    return flask.redirect(flask.url_for("save"))
...
@app.route("/delete", methos=["POST"])
def delete():
    artist_id = flask.request.form.get("artist_id")
    username = flask.request.form.get("username")
    Artist.query.filter_by(artist_id.like(artist_id), username.like(username)).delete()
    return flask.redirect(flask.url_for("save"))
@app.route("/")
def main():
